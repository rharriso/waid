...
func add() {
  ...
  e.SetDuration(*dur)
  jsonRequest("http://localhost:3000/entries", "post", &e)
...
func list() {
  ...
  jsonRequest("http://localhost:3000/entries", "get", &entries)
...
*/
func jsonRequest(path string, reqType string, v interface{}) {
  var err error
  ...
  case "post":
    ...
    doPanic(err)
    resp, err = http.Post(path, "json", bytes.NewBuffer(jsonData))
    doPanic(err)
  }

  // set returned data to interface, this will crash if the types aren't good
  doPanic(err)
